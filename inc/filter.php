...
{
    echo get_projects_filter_form(array('project_areas'));
}
...
function get_projects_filter_form($taxonomies = array())
{
    ...
    foreach ($taxonomies as $taxonomy) {
        ...
        if (!empty($terms) && !is_wp_error($terms)) {
            ...
            foreach ($terms as $term) {
                ...
                $form .= '<input type="radio" name="' . esc_attr($taxonomy) . '" id="' . esc_attr($taxonomy . '_' . $term->slug) . '" value="' . esc_attr($term->slug) . '" ' . esc_attr($checked) . '>';
    ...
    $form .= '<button type="submit">Filter</button>';
    $form .= '</form>';
